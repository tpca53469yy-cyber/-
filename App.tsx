  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-12 select-none overflow-x-hidden font-sans">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-30 pt-[env(safe-area-inset-top)]">
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <button 
            ...
            className={`flex items-center space-x-2 px-4 py-2 rounded-full text-[10px] font-black transition-all active:scale-90 ${
              isLiveMode 
          ...
          <div className="relative group">
            ...
            <div className="relative bg-slate-900 rounded-[3rem] p-8 shadow-2xl overflow-hidden min-h-[300px] flex flex-col justify-between border border-white/10">
              <div className="flex justify-between items-center mb-8">
                <div className="bg-orange-500/10 border border-orange-500/20 px-3 py-1 rounded-full text-orange-400 text-[9px] font-black tracking-widest uppercase">
                  Real-time Support
                </div>
                <div className="flex items-end space-x-1 h-6">
                  {[...Array(12)].map((_, i) => (
                    <div 
                      ...
                      style={{ height: `${Math.random() * (audioLevel / (i+1) + 10)}%` }}
                    ></div>
                  ))}
                </div>
              </div>
              <div className="space-y-6">
                <h2 className="text-white text-2xl font-bold leading-tight animate-in fade-in slide-in-from-bottom-2 duration-700">
                  {liveAdvice}
                </h2>
                <div className="flex flex-wrap gap-2">
                  <div className="bg-white/5 border border-white/10 px-4 py-2 rounded-2xl text-[10px] text-slate-400 font-bold">
                    <i className="fa-solid fa-shield-halved mr-2 text-blue-400"></i> 抗噪模式已啟動
                  </div>
                </div>
              </div>
        ...
        {!isLiveMode && (streamingText || result) && (
          <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="bg-white border-2 border-orange-400 rounded-[3rem] p-8 shadow-2xl shadow-orange-100/50 relative">
              ...
              <p className="text-2xl sm:text-3xl font-black text-slate-800 leading-[1.3] mt-2">
                「{streamingText}」
              </p>
              {result && (
          ...
          <div className={`bg-white rounded-[3rem] shadow-sm border transition-all duration-500 overflow-hidden ${isListening ? 'border-red-400 ring-[6px] ring-red-50' : 'border-slate-200'}`}>
            <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-100 flex space-x-3 overflow-x-auto no-scrollbar scroll-smooth">
              {SCENARIOS.map((s) => (
                <button
                  key={s.type}
                  onClick={() => { setSelectedScenario(s.type); triggerHaptic('light'); }}
                  className={`whitespace-nowrap px-5 py-2.5 rounded-2xl text-[11px] font-black transition-all active:scale-90 ${
                    selectedScenario === s.type ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-slate-400 border border-slate-100'
                  }`}
                >
            ...
            <div className="p-8">
              <textarea
                ...
                placeholder={isListening ? '我正在傾聽您的感受...' : currentPlaceholder}
                className={`w-full transition-all duration-300 text-xl font-medium border-0 bg-transparent focus:ring-0 outline-none resize-none placeholder:text-slate-200 ${result ? 'h-24' : 'h-40'}`}
              />
              ...
              <div className="flex justify-between items-center pt-6 border-t border-slate-50">
                ...
                <div className="flex space-x-4">
                  <button
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all active:scale-90 ${isListening ? 'bg-red-500 text-white animate-pulse shadow-xl shadow-red-200' : 'bg-slate-100 text-slate-600'}`}
                  >
                  ...
                  <button
                    ...
                    className="px-8 bg-orange-500 text-white rounded-full font-black text-sm shadow-xl shadow-orange-200 disabled:bg-slate-100 disabled:text-slate-300 transition-all active:scale-95"
                  >
        ...
        {history.length > 0 && !isLiveMode && (
          <section className="space-y-4">
            <div className="flex justify-between items-end px-2">
               ...
               <button 
                onClick={() => { setHistory([]); localStorage.removeItem('parenting_history'); triggerHaptic('light'); }}
                className="text-[10px] text-slate-300 font-bold hover:text-red-400"
              >
                清空全部
              </button>
            </div>
            <div className="grid grid-cols-1 gap-4">
              {history.map((h, i) => (
                <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col space-y-2 animate-in slide-in-from-left-4 duration-300" style={{ animationDelay: `${i * 50}ms` }}>
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-1">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed">
                我們整合了全球公認的三大教養體系，將冰冷的心理學轉化為您口袋裡的溫柔力量。
              </p>
            </div>
      ...
      <footer className="mt-16 py-12 text-center px-10 border-t border-slate-100 mb-[env(safe-area-inset-bottom)]">
        ...
        <p className="text-[11px] text-slate-400 leading-loose max-w-xs mx-auto">
          「沒有任何一種翻譯，比您的愛更有力量。」<br/>
          這是一個專為您與孩子設計的避風港。
        </p>
      </footer>
    </div>
